...
collaborator guide](WCS-GUIDE.md).
...
[`.env.example`](.env.example) to `.env` (git-ignored) and fill in your
values. `read_config()` loads it and reads
[`inst/config.yml`](inst/config.yml). The `default` profile uses the
development buckets and databases. The `production` profile is switched
on only by CI on `main`.
...
`validate_kefs_surveys_v2()`. The order of the steps is in
...
[`build-container.yaml`](.github/workflows/build-container.yaml), which
installs the latest `coasts` release. Runs on `main` use the
`production` profile; runs on any other branch use `default`.

| Workflow | When it runs | What it does |
|----|----|----|
| [`data-pipeline.yaml`](.github/workflows/data-pipeline.yaml) | 00:00 UTC on every second day of the month (1st, 3rd, 5th, …), and on every push | The full pipeline: WCS, KEFS and GPS data |
| [`wcs-pipeline.yaml`](.github/workflows/wcs-pipeline.yaml) | On demand, and on push to the WCS code | The WCS chain alone, writing to development storage only |
...
[`NEWS.md`](NEWS.md) headed `# peskas.kenya.data.pipeline X.Y.Z`,
written for non-technical readers. On push to `main`, `release.yaml`
