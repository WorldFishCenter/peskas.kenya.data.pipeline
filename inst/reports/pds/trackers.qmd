...
```{r data-load}
# Load vessel tracking data (updated with latest trips)
tracks_binned <- 
arrow::read_parquet("tracks_binned_07-11.parquet") %>%
dplyr::filter(lng > 38.18)
...
```{r vessel-interactions}
# Simple approach: Find where vessels interact (2+ vessels in same location)
vessel_interactions <- tracks_processed %>%
...
interaction_zones <- vessel_interactions %>%
  group_by(lat_grid, lng_grid) %>%
  summarise(
    total_events = n(),
...
cat("Zones with interactions:", nrow(interaction_zones), "\n")
cat("Total interaction events:", sum(interaction_zones$total_events), "\n")
cat("Max vessels in one spot:", max(interaction_zones$max_vessels), "\n\n")

# Create simple gradient map (yellow to dark red)
pal_interaction <- colorNumeric(
  palette = colorRampPalette(c("#FFFFCC", "#FFEDA0", "#FED976", "#FEB24C",
                                "#FD8D3C", "#FC4E2A", "#E31A1C", "#BD0026"))(100),
  domain = interaction_zones$total_vessel_hours
)

leaflet(interaction_zones, height = "700px") %>%
  ...
  addCircles(
    lng = ~lng_grid,
    lat = ~lat_grid,
    radius = ~ total_vessel_hours * 100,
    fillColor = ~ pal_interaction(total_vessel_hours),
    fillOpacity = 0.7,
    weight = 1,
    color = "#FFFFFF",
    popup = ~ paste(
      "<b>Interaction Zone</b><br>",
      "<b>Location:</b>", round(lat_grid, 3), ",", round(lng_grid, 3), "<br>",
      "<b>Times vessels met here:</b>", total_events, "<br>",
      "<b>Average vessels:</b>", round(avg_vessels, 1), "<br>",
      "<b>Peak vessels:</b>", max_vessels, "<br>",
      "<b>Total vessel-hours:</b>", total_vessel_hours
    ),
    label = ~ paste(total_vessel_hours, "vessel-hours")
  ) %>%
  addLegend(
    position = "bottomright",
    pal = pal_interaction,
    values = ~total_vessel_hours,
    title = "Interaction<br>Intensity",
    labFormat = labelFormat(suffix = " vessel-hrs"),
    opacity = 0.8
  ) %>%
  addControl(
    html = "<div style='background: rgba(255,255,255,0.9); padding: 10px; border-radius: 5px; color: black;'>
            <h4>Vessel Interaction Zones</h4>
            <p><strong>Darker red</strong> = More vessel activity</p>
            <p><strong>Circle size</strong> = Total vessel-hours</p>
...
# # Top 10 zones
# top_zones <- interaction_zones %>%
#   arrange(desc(total_vessel_hours)) %>%
#   head(10) %>%
#   mutate(zone_id = paste0("Zone ", row_number()))

# highchart() %>%
#   hc_add_series(
#     data = top_zones,
#     type = "bar",
#     hcaes(x = zone_id, y = total_vessel_hours),
#     name = "Vessel-Hours",
#     color = "#E31A1C"
#   ) %>%
#   hc_title(text = "Top 10 Vessel Interaction Hotspots") %>%
#   hc_subtitle(text = "Areas with most vessel overlap") %>%
#   hc_xAxis(title = list(text = ""), categories = top_zones$zone_id) %>%
#   hc_yAxis(title = list(text = "Total Vessel-Hours")) %>%
#   hc_tooltip(pointFormat = "<b>{point.y}</b> vessel-hours")

# Daily pattern
# daily_interactions <- vessel_interactions %>%
#   group_by(date) %>%
#   summarise(
#     daily_events = n(),
#     .groups = "drop"
...
#     type = "area",
#     hcaes(x = date, y = daily_events),
#     name = "Interactions",
#     color = "#2E86AB",
#     fillOpacity = 0.3
#   ) %>%
#   hc_title(text = "Daily Vessel Interactions Over Time") %>%
#   hc_subtitle(text = "How often vessels share the same fishing areas") %>%
#   hc_xAxis(type = "datetime", title = list(text = "Date")) %>%
#   hc_yAxis(title = list(text = "Number of Interaction Events"))
```
...
- **Temporal patterns** can inform timing of fisher consultations and resource assessments
- **Communication systems** between vessels could enhance cooperation and information sharing
- **Shared areas** present opportunities for co-management agreements among user groups

::: key-finding
### 🤝 **Vessel Interaction Summary**

-   **`r nrow(interaction_zones)`** areas where vessels interact
-   **`r sum(interaction_zones$total_events)`** total interaction events recorded
-   Maximum **`r max(interaction_zones$max_vessels)`** vessels observed together in one location
-   Average **`r round(mean(interaction_zones$avg_vessels), 1)`** vessels per interaction event
:::
...
apex(
  ...
  ax_xaxis(
    title = list(text = "Hour of Day (24-hour format)"),
    min = 0,
    max = 23
  ) %>%
...
```{r reg-confl}
# Regional vessel interaction analysis
regional_conflicts <- vessel_interactions %>%
  left_join(
    tracks_processed %>%
  ...
  columns = list(
    ...
    total_conflicts = colDef(
      ...
      style = function(value) {
        ...
        if (value > 20) {
          list(color = "#2e7d32", fontWeight = 500)
        } else if (value > 10) {
          list(color = "#388e3c")
        } else {
          list(color = "#66bb6a")
...
**Vessel Interactions:** - Most interaction events: **`r if(nrow(regional_conflicts) > 0) regional_conflicts$region[which.max(regional_conflicts$total_conflicts)] else "N/A"`** with **`r if(nrow(regional_conflicts) > 0) max(regional_conflicts$total_conflicts) else 0`** events - Highest interaction intensity: **`r if(nrow(regional_conflicts) > 0) regional_conflicts$region[which.max(regional_conflicts$avg_vessels_per_conflict)] else "N/A"`** averaging **`r if(nrow(regional_conflicts) > 0) round(max(regional_conflicts$avg_vessels_per_conflict), 1) else 0`** vessels per interaction event
:::
...
Rather than categorizing interactions into discrete levels, we use a **continuous color gradient**:
- Light yellow indicates low interaction intensity (few vessel-hours)
- Dark red indicates high interaction intensity (many vessel-hours)
- Circle size on the map is proportional to the square root of vessel-hours for better visual scaling
