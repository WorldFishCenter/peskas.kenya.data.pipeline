...
1. **Fishers vs. Boats**  
   - A submission is flagged if `no_of_fishers < n_boats`.  
2. **Gear Requiring Boats**  
   - Certain gears (e.g., _gillnet, handline, traps, monofilament, reefseine, ringnet, setnet_) require at least one boat. If `n_boats == 0` and the gear is in this list, the submission is flagged.  
3. **Negative Catch**  
   - Any record with a negative total catch is flagged.  
4. **Non-positive Effort**  
   - Submissions with zero or negative fishers, or negative boat values, are flagged.  
5. **Zero Catch With Non-zero Effort**  
   - Having a total catch of zero while reporting one or more fishers or boats is illogical and gets flagged.

Records that fail any of these checks are removed from the dataset prior to further validation and outlier screening.
