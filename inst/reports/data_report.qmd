...
format:
  lumo-html: 
    #logo: "https://github.com/WorldFishCenter/peskas.kenya.data.pipeline/raw/main/man/figures/logo.png"                               
...
```{r load_initial, warning=FALSE, message=FALSE, include=FALSE, cache=TRUE, dependson="setup"}
# Remove library loads from here and keep only data loading
...
- Convert the data into a tidy format, where each column represents a single variable, each row corresponds to one observation, and all variables have the same unit of observation. (For more information, see [Tidying Data](https://dimewiki.worldbank.org/Tidying_Data)).
...
In the histogram below, the dashed line indicates the outlier threshold at `r round(upb, 2)` boats, determined by the data using a k value of 1.5. Adjusting k changes sensitivity: lower k detects more outliers, while higher k detects fewer.
...
The following plot shows survey activity across BMUs from `r format(min(valid_data$landing_date, na.rm = T), '%B %Y')` to `r format(as.Date("2022-10-31"), '%B %Y')`. Hover over tiles to view the number of surveys conducted in specific quarters and landing sites:
...
::: {.cell .page-columns .page-full .column-screen .blue-section}
...
```{r catch_plot, warning=FALSE, message=FALSE, fig.height=8,dependson="load_initial"}
